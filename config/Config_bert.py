...
class Config(object):
    ...
    def from_list_to_tensor(self, item, train=True):
        ...
        sen_matrix = np.zeros((node_num, node_num, graph.graph['max_sentence_num'],
                               len(item['document'])))  # n*n*n_sentence*document_length
        pos_matrix_h = np.zeros((node_num, node_num, graph.graph['max_sentence_num'],
                                 len(item['document'])))  # n*n*n_sentence*document_length
        pos_matrix_t = np.zeros((node_num, node_num, graph.graph['max_sentence_num'],
                                 len(item['document'])))  # n*n*n_sentence*document_length
    ...
    def load_train_data(self):
        ...
        for i in range(self.train_file_size):
            self.train_file.extend(
                pickle.load(open(os.path.join(self.data_path, prefix + str(i) + '.pkl'), 'rb')))  # load pickle文件，文件过大
    ...
    def load_test_data(self):
        ...
        for i in range(self.test_file_size):
            ...
            # self.test_file = pickle.load(open('./new_data/dev_test0.pkl', 'rb'))  # test set
            self.test_file.extend(pickle.load(open(os.path.join(self.data_path, prefix + str(i) + '.pkl'), 'rb')))
    ...
    def train(self, model_pattern, model_name):
        ...
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()),
                               lr=self.learn_rate)  # ,weight_decay=1e-6)   #只放required_grad
        ...
        for epoch in range(self.max_epoch):
            ...
            for i_data in tqdm(range(len(self.train_file))):  # 一个batch的数据
                ...
                predict_re = model(document, document_ner, document_pos, adj_matrix, sen_matrix, pos_matrix_h,
                                   pos_matrix_t, node_pos, node_type, node_relative_pos)
            ...
            logging(
                '| epoch {:2d} | ms/b {:5.2f} | train loss {:5.3f} | NA acc: {:4.2f} | not NA acc: {:4.2f}  | tot acc: {:4.2f} '.format(
                    epoch, elapsed * 1000 / len(self.train_file), float(all_loss), self.acc_NA.get(),
    ...
    def test(self, model, model_name, output=False, input_theta=-1):
            ...
            for i in range(predicts.shape[0]):  # predicts.shape[0]和predicts.shape[1] 就是结点/实体的数量
                for j in range(predicts.shape[1]):
                    ...
                    for k in range(self.relation_num):
                        ...
                        test_result.append((labels[i, j, k], float(predicts[i, j, k]), in_train, index, i, j,
                                            k))  # , self.id2rel[k], i, j, k) )
        ...
        if not self.is_test:
            logging(
                'ALL  : Theta {:3.4f} | P {:3.4f} | R {:3.4f} | F1 {:3.4f} | AUC {:3.4f}'.format(theta, p, r, f1, auc))
        else:
            logging('ma_f1 {:3.4f} | input_theta {:3.4f} test_result F1 {:3.4f} | AUC {:3.4f}'.format(f1, input_theta,
                                                                                                      f1_arr[w], auc))
        output = True
        if output:
            output = [{'index': x[3], 'h_idx': x[4], 't_idx': x[5], 'r_idx': x[6], 'r': self.id2rel[x[6]],
                       'title': self.test_file[x[3]]['title']} for x in test_result[:w + 1]]
    ...
    def testall(self, model_pattern, model_name, input_theta):  # , ignore_input_theta):
        ...
        model.load_state_dict(
            torch.load(os.path.join(self.checkpoint_dir, model_name)))  # self.checkpoint_dir = './checkpoint'
