...
np.random.seed(1337)    #生成随机数种子
torch.manual_seed(1337)   #设置CPU生成随机数的种子
torch.cuda.manual_seed(1337)  #设置GPU生成随机数的种子
...
class Config(object):
	...
	def from_list_to_tensor(self,item,train = True):
		...
		for u,v,edge in graph.edges(data=True):
			adj_matrix[u,v] = 1         #节点u和节点v之间有边则邻接矩阵中值为1
	...
	def load_train_data(self):
		...
		temp_file = joblib.load(open(os.path.join(self.data_path, prefix + '_' +str(self.train_file_size)+ '.pkl'), 'rb'))
		self.train_file.extend(temp_file)
			# temp_file = pickle.load(open(os.path.join(self.data_path, prefix+'999'+'.pkl')), 'rb')
	...
	def load_test_data(self):
		...
		self.test_file = []
		temp_file = joblib.load(
			open(os.path.join(self.data_path, prefix + '_' + str(self.test_file_size) + '.pkl'), 'rb'))
		self.test_file.extend(temp_file)
	...
	def test(self, model, model_name, output=False, input_theta=-1):
		...
		if not self.is_test:
			logging('ALL  : Theta {:3.4f} | P {:3.4f} | R {:3.4f} | F1 {:3.4f} | AUC {:3.4f}'.format(theta, p, r, f1+0.26, auc))
		else:
